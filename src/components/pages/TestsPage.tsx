...
// Fix: Added a color variants map to avoid Tailwind JIT issues with dynamic classes.
...
interface TestCardProps {
    ...
    duration: string;
    questions: number;
    color: keyof typeof testCardColorVariants;
}

const TestCard: React.FC<TestCardProps> = ({ icon: Icon, title, description, duration, questions, color }) => {
    const variant = testCardColorVariants[color];
    return (
        <div className="bg-card border rounded-lg p-6 flex flex-col">
            <div className="flex items-start gap-4">
                <div className={`p-3 rounded-full ${variant.iconContainer}`}>
                    <Icon className={`h-8 w-8 ${variant.icon}`} />
                </div>
                <div>
            ...
            <button className={`mt-4 w-full font-semibold py-2 rounded-lg ${variant.button} text-white/90 hover:opacity-90 transition-opacity`}>
                Iniciar Teste
            </button>
        </div>
    );
